...
## [0.17.2] - 2026-07-03
### Changed
- **Manage-popup verb row is now Reassign / Unassign / Delete** (dev1's final verb model; `Change value` stays the primary write above it): **Unassign** is what 0.17.1's `Delete` did - it removes ONLY the registry entry, the vault is untouched (`Unassign secret?` confirm, same consequences text); **Delete** now actually deletes the VAULT ITEM (dev1's bug report: after Delete he expected `build-plugin-test_steam-user` gone from the vault) AND removes the assignment with it - behind a `Delete vault secret?` confirm that names the item verbatim and adds a CAUTION when the item looks SHARED (another registry entry points at the same item, or its bare un-prefixed title marks it team-level - deleting may break other consumers). Vault delete runs FIRST: on failure nothing is removed and the error is surfaced. Delete needs the provider (disabled with the reason when signed out or the item title is unrecoverable); Unassign still works signed-out.
- **New mandatory provider verb `DeleteItemAsync(item)`** (`ISecretProvider`): deletes the whole vault item by verbatim title, document items too; `OnePasswordProvider` implements it via `op item delete` - the self-service lifecycle (create, rotate, repoint, unassign, delete) is now fully panel-side, mirrored in `provider-contract.md`.
- **The Secrets view's row `Remove` button is renamed `Unassign`** (same entry-only semantics, same shared confirm builder - `SecretDemand.UnassignConfirmMessage`, ex `RemoveConfirmMessage`) and now appears ONLY on `OK - unused` rows: a needed-registered row's Unassign lives in the Edit popup's verb row instead of a competing row shortcut.
...
- **Field picker offers only real fields**: the register dialog's use-existing mode no longer lists 1Password's built-in EMPTY fields (e.g. the blank `password` on a Login/Password-category item) - only fields that actually hold a value are offered (values are checked for length only, discarded immediately, never rendered). A single remaining field is preselected; an item with no readable fields says so instead of offering phantoms.
- **Button/menu label cleanup** (dev1): no more `...` suffixes anywhere in the Editor UI - `Register`, `Fix value`, `Register <KEY>`, `Register new`, `Change reference`, `Browse`; the rotation button is renamed `Set value...` -> `Change value` (same write-only rotation semantics). Status texts are unchanged.
...
- **Naming convention unified across the checkout path** (project-setup wizard cut over, matching the already-live agent scripts + vault): the per-project VCS record is `<project-key>_vcs` (was `vcs-<key>`; named in one place, `SecretProvisioner.VcsRecordNameFor`); its `credentialName` field stores the credential item's **verbatim title** - the hidden `cred-` prefix (stripped on display, re-applied by the agent) is deleted everywhere. New per-repo deploy keys are auto-named `<project>_<host>-deploy-key` (per-repo by nature - GitHub rejects one key on two repos); shared/reusable credentials (bot SSH keys, Plastic user/pass, UVCS PATs) keep bare user-chosen `<host>-<account>` names. Select-existing enumerates the vault without a prefix filter and offers item titles verbatim (the plugin's own `*_vcs` / `unity-licenses` registry items excluded).
